<nav class="navbar sticky-top navbar-expand-lg navbar-dark bg-dark">
        ...
        <div class="collapse navbar-collapse navbar-style" id="navbarSupportedContent">
            ...
            <?php if(isset($_GET['PIC_id'])){ 
              $PIC_id = $_GET['PIC_id'];
              $query = "SELECT * FROM teamlist WHERE  team_id = $PIC_id";
              $select_team = mysqli_query($connection,$query);
              while($row = mysqli_fetch_assoc($select_team)){
                  $PIC_post = $row['team_name'];
              }   
         
          ?>
          <form class="d-flex" action="searchPIC_post.php?team_name=<?php echo $PIC_post ?>" method="post" >
