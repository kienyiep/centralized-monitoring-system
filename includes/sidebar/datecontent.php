...
<?php
             foreach($array_expiry as $date_object){
               $expiry_get = $date_object->expiry_object;
               $renewal_get = $date_object->renewal_object;

            
                $query = "SELECT * FROM {$team_name} WHERE license_expiry LIKE '%$expiry_get%' AND license_renewal LIKE '%$renewal_get%' ORDER BY license_id DESC";
               
                $select_all_license_query = mysqli_query($connection, $query);
                check_query($select_all_license_query); 
                
                while($row = mysqli_fetch_array($select_all_license_query)){
